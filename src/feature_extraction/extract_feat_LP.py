...
class ZScoreNormalizationd(MapTransform):
    ...
    def __call__(self, data):
        ...
        for key in self.key_iterator(d):
            ...
            #non_zero_mask = (channel_data > channel_data.min()).astype(np.float32)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Extracting features")):
            ...
            image_embeddings = torch.concat([torch.nn.functional.adaptive_avg_pool3d(output, 1) for output in outputs], dim=1)
