...
from nnssl.utilities.file_path_utilities import *
    ...
    with zipfile.ZipFile(output_file, "w", zipfile.ZIP_DEFLATED) as zipf:
        for c in configurations:
            ...
            expected_fold_folder = [
                f"fold_{i}" if i != "all" else "fold_all" for i in folds
            ]
            assert all(
            ...
            for fold_folder in expected_fold_folder:
                ...
                if isfile(source_file):
                    zipf.write(source_file, os.path.relpath(source_file, nnssl_results))
                ...
                if export_crossval_predictions:
                    ...
                    for f in files:
                        zipf.write(
                            ...
                            os.path.relpath(join(source_folder, f), nnssl_results),
                        )
            ...
            if isdir(source_folder):
                ...
                for s in source_files:
                    if isfile(s):
                        zipf.write(s, os.path.relpath(s, nnssl_results))
            # plans
        ...
        ensemble_dir = join(nnssl_results, dataset_name, "ensembles")
        ...
        inference_information_file = join(
            nnssl_results, dataset_name, "inference_information.json"
        )
        if isfile(inference_information_file):
            zipf.write(
                inference_information_file,
                os.path.relpath(inference_information_file, nnssl_results),
            )
        inference_information_txt_file = join(
            nnssl_results, dataset_name, "inference_information.txt"
        )
        if isfile(inference_information_txt_file):
            zipf.write(
                inference_information_txt_file,
                os.path.relpath(inference_information_txt_file, nnssl_results),
            )
    print("Done")
