...
class SpatialTransform(AbstractTransform):
    ...
    def __init__(self, patch_size, patch_center_dist_from_border=30,
                 ...
                 ## NOTE: added to first crop to approx foreground
                 first_crop_foreground_size: int=None,
                 ):
        ...
        self.first_crop_foreground_size = first_crop_foreground_size

    def __call__(self, **data_dict):
        data = data_dict.get(self.data_key)
        seg = data_dict.get(self.label_key)
        # data and seg shape same
        #assert data.shape == seg.shape, "data and seg must have the same shape!"
        #print(data.shape)
        ...
        # # NOTE: first crop to foreground region
        # if self.first_crop_foreground_size is not None:
        #     import pdb; pdb.set_trace()
        #     # size_tuple = (self.first_crop_foreground_size,) * len(patch_size)
        #     # support only 3d data (1, bs)
        #     # center crop (bs, c, d, h, w) to (bs, c, d, self.first_crop_foreground_size, self.first_crop_foreground_size)
        #     height_start = (data.shape[3] - self.first_crop_foreground_size) // 2
        #     width_start = (data.shape[4] - self.first_crop_foreground_size) // 2
        #     height_end = height_start + self.first_crop_foreground_size
        #     width_end = width_start + self.first_crop_foreground_size
        #     data = data[:, :, :, height_start:height_end, width_start:width_end]
        #     seg = seg[:, :, :, height_start:height_end, width_start:width_end]

        # NOTE: added for random patch generation (center distance from border = patch_size//2)
        if self.patch_center_dist_from_border is None and self.random_crop is True:
            #center_dist_from_border = [x // 2 for x in patch_size]
            data_shape = data.shape
            center_dist_from_border = [x // 2 for x in patch_size] if self.first_crop_foreground_size is None else [data_shape[2]//2, data_shape[3]//2, data_shape[4]//2]
